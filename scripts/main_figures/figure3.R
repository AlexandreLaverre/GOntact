...
if(prepare){
  ...
  for(sp in species){
    ...
    for(sample in names(enrichment.results[[sp]])){
      ...
      signif.great1Mb=test.great1Mb$GOID[which(test.great1Mb$FDR<maxFDR)]
      signif.contacts1Mb=test.contacts1Mb$GOID[which(test.contacts1Mb$FDR<maxFDR)]
