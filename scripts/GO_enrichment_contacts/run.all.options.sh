#!/bin/bash
#Contacts options
export ExtendOverlap_options=(5000)
export maxDistance_options=(1000000)
export minDistance_options=(0 25000)
export BackgroundEnhancers_options=(FANTOM5.Laverre2022.bed)

#GOFrequency options
export GONameSpace_options=(biological_process)
...
./foreground.and.background.contacts.py human FANTOM5.first1000.kidney.enhancers.hg38.bed --ExtendOverlap ${ExtendOverlap} --maxDistance ${maxDistance} --minDistance ${minDistance} --BackgroundEnhancers ${BackgroundEnhancer}
...
if [[ "${BackgroundEnhancer}" == "FANTOM5.Laverre2022.bed" ]] && [[ "${EnhancerContact}" == "--EnhancerContact" ]]; then
echo "${BackgroundEnhancer}"
