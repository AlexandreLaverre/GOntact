...
let get_bait_coords_path_or_die pl =
...
module Enrich = struct
  ...
  let save_params pl path =
    Sexp.save_hum path (sexp_of_params pl)
  ...
  let write_elements pl label dist_elements elements_by_gocat =
  ...
  let write_enrichment pl ~gonames ~annotated_baits ~propagated_fa enriched_terms =
    let bait_collection = annotated_baits.Contact_enrichment_analysis.baits in
  ...
  let write_symbol_annotated_baits pl annotated_baits filtered_annot =
    let bait_collection = annotated_baits.Contact_enrichment_analysis.baits in
    let symbol_annotated_baits = Chromatin_contact.symbol_annotate_baits ~bait_collection ~genome_annotation:filtered_annot ~max_dist:pl.common.max_dist_bait_TSS in
  ...
  let contacts_mode pl ~gonames ~filtered_annot ~foreground ~background ~contact_graph ~propagated_fa ~annotated_baits =
    let { Contact_enrichment_analysis.contacted_fragments ; fragment_to_baits ;
          element_annotation ; enriched_terms ; _ } =
      Contact_enrichment_analysis.perform
        ~margin:pl.common.max_dist_element_fragment
        annotated_baits
        propagated_fa
        contact_graph
        { FGBG.foreground ; background }
    in
    write_enrichment pl ~gonames ~annotated_baits ~propagated_fa enriched_terms ;

    if (pl.write_elements_foreground || pl.write_elements_background) then (
      let major_isoforms = Utils.chrono "extract major isoforms symbols" Genomic_annotation.identify_major_isoforms_symbols filtered_annot in
      let symbol_annotated_baits = write_symbol_annotated_baits pl annotated_baits filtered_annot in
      let gene_distance_by_element elements =
        contact_gene_distance_by_element
          elements ~filtered_annot ~major_isoforms
          ~contacted_fragments ~fragment_to_baits ~symbol_annotated_baits
      in
      let elements_by_gocat annotation =
        let gocat_by_element = expand_go_term_sets annotation propagated_fa in
        Chromatin_contact.elements_by_annotation gocat_by_element
      in
      if pl.write_elements_foreground then
        write_elements pl "foreground" (gene_distance_by_element foreground) (elements_by_gocat element_annotation.foreground) ;
      if pl.write_elements_background then
        write_elements pl "background" (gene_distance_by_element background) (elements_by_gocat element_annotation.background) ;
    )
  ...
  let hybrid_mode pl ~chromosome_sizes ~filtered_annot ~foreground ~background ~contact_graph ~propagated_fa ~gonames ~annotated_baits =
    let { upstream ; downstream ; _ } = pl.common in
    let great_param = { Great.upstream ; downstream ; extend = 0 } in
    ...
    if (pl.write_elements_foreground || pl.write_elements_background) then (
      ...
      in
      if pl.write_elements_foreground then
        write_elements pl "foreground" (gene_distance_by_element foreground) (elements_by_gocat contacts.element_annotation.foreground great.element_annotation.foreground) ;
      if pl.write_elements_background then
        write_elements pl "background" (gene_distance_by_element background) (elements_by_gocat contacts.element_annotation.background great.element_annotation.background)
    )

  let contact_data_prepare pl ~genome_annotation ~functional_annotation =
    let bait_collection =
      Genomic_interval_collection.of_bed_file (get_bait_coords_path_or_die pl) ~strip_chr:true ~format:Base1 in
    let annotated_baits =
    ...
    let contact_graph =
      Contact_enrichment_analysis.aggregate_contact_graphs
        ~bait_annotation:annotated_baits.annotation
        bait_collection
        contact_graphs
        {
  ...
  let main ({
      ...
      fg_path ; bg_path ;
      functional_annot ; obo_path ; domain ; _
    } as params) =
    let main_result =
  ...
  let term =
    ...
    let pl = { common ;
               fg_path ; bg_path ;
               functional_annot ; obo_path ; domain ;
  ...
  let info = Cmd.info ~doc:"Compute GO enrichments" "enrich"
  let command = Cmd.v info term
end

module Annotate = struct
  let make_contact_graph pl bait_collection =
    let contact_graphs =
      List.map pl.ibed_files ~f:(Chromatin_contact_graph.of_ibed_file ~strip_chr:true)
    in
    Contact_enrichment_analysis.aggregate_contact_graphs
  ...
  let main
    ...
    let dist_elements = match mode with
      | Hybrid ->
        let bait_collection = Genomic_interval_collection.of_bed_file (get_bait_coords_path_or_die pl) ~strip_chr:true ~format:Base1 in
        let contact_graph = make_contact_graph pl bait_collection in
        let contacted_fragments = Chromatin_contact_graph.extend_fragments contact_graph ~margin:pl.max_dist_element_fragment in
        let chromosome_sizes = Genomic_interval_collection.of_chr_size_file chr_sizes ~strip_chr:true in
        let domains = Great.(domains_intervals { upstream ; downstream ; extend } ~chromosome_sizes ~genome_annotation) in
        let fragment_to_baits = Chromatin_contact_graph.fragment_to_baits contact_graph in
        let symbol_annotated_baits = Chromatin_contact.symbol_annotate_baits ~bait_collection ~genome_annotation ~max_dist:pl.max_dist_bait_TSS in
        hybrid_gene_distance_by_element elts
          ~domains ~contacted_fragments ~filtered_annot:genome_annotation
          ~fragment_to_baits ~symbol_annotated_baits ~major_isoforms
      | GREAT ->
        let chromosome_sizes = Genomic_interval_collection.of_chr_size_file chr_sizes ~strip_chr:true in
        let domains = Great.(domains_intervals { upstream ; downstream ; extend } ~chromosome_sizes ~genome_annotation) in
        great_gene_distance_by_element elts ~domains ~filtered_annot:genome_annotation ~major_isoforms
      | Contacts ->
        let bait_collection = Genomic_interval_collection.of_bed_file (get_bait_coords_path_or_die pl) ~strip_chr:true ~format:Base1 in
        let contact_graph = make_contact_graph pl bait_collection in
        let contacted_fragments = Chromatin_contact_graph.extend_fragments contact_graph ~margin:pl.max_dist_element_fragment in
        ...
        let symbol_annotated_baits = Chromatin_contact.symbol_annotate_baits ~bait_collection ~genome_annotation ~max_dist:pl.max_dist_bait_TSS in
        contact_gene_distance_by_element
          elts ~filtered_annot:genome_annotation ~major_isoforms
          ~contacted_fragments ~fragment_to_baits ~symbol_annotated_baits
    in
    Genomic_annotation.write_distance_elements ~dist_elements output_path
