...
let combine_maps gomap1 gomap2 =
  let goids1 = Map.keys gomap1 in
  let goids2 = Map.keys gomap2 in
  ...
  let gotuples = List.map unique_goids ~f:(fun goid ->
      let olist1 = Map.find gomap1 goid in
      let olist2 = Map.find gomap2 goid in
      let l = (
...
let go_frequencies ~categories_by_element fa =
  ...
  let counts =
    ...
    Ontology.PKey.Table.fold table ~init:String.Map.empty ~f:(fun ~term ~value acc ->
        if value > 0 then Map.add_exn acc ~key:term.id ~data:value
        else acc
      )
  in
  Map.add_exn counts ~key:"total" ~data:nb_total

let foreground_vs_background_binom_test ~go_frequencies_foreground ~go_frequencies_background =
  let total_bg = Map.find_exn go_frequencies_background "total" in
  let total_fg = Map.find_exn go_frequencies_foreground "total" in
  let ids_bg = Map.keys go_frequencies_background in
  let go_ids_bg = String.Set.of_list (List.filter ids_bg ~f:(fun id -> not (String.equal id "total"))) in
  let ids_fg = Map.keys go_frequencies_foreground in
  let go_ids_fg = String.Set.of_list (List.filter ids_fg ~f:(fun id -> not (String.equal id "total"))) in
  let shared_go_ids = Set.to_list (Set.inter go_ids_bg go_ids_fg) in
  let test_one_category id = (*by construction id has to be present for both fg and bg set *)
    let count_fg = Map.find_exn go_frequencies_foreground id in
    let count_bg = Map.find_exn go_frequencies_background id in
  ...
  let construct_enrichment_object id =
    let count_fg = Map.find_exn go_frequencies_foreground id in
    let count_bg = Map.find_exn go_frequencies_background id in
    let pval = Map.find_exn all_pvalues_dict id in
    let fdr = Map.find_exn all_fdr_dict id in
...
let write_output results go_names path =
  ...
  Out_channel.with_file path ~append:false ~f:(fun output ->
      ...
      List.iter ordered_results  ~f:(fun res ->
          let name = Map.find_exn go_names res.id in
...
let combine_annotations xs ys =
  ...
  Map.fold2 (to_map xs) (to_map ys) ~init:[] ~f:(fun ~key:_ ~data acc ->
      match data with
      | `Left _ | `Right _ -> acc
