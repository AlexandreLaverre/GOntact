...
let main ~bait_coords ~gene_info ~max_dist ~output_path =
  ...
  | Ok ga -> (
      ...
      let tss_int = Genomic_annotation.all_tss_intervals filtered_annot_bio_tx max_dist in
...
let term =
  ...
  in
  main ~bait_coords ~gene_info ~max_dist ~output_path
